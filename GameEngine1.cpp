    ...
    while (true) {
        ...
        if (!file) {
            ...
            while (getline(file, line)) {
                ...
                Veggie *currVeggie = new Veggie(symbol, name, stoi(point));
...
void GameEngine::initRabbits() {
    ...
    while (restOfRabbits != 0) {
        ...
        int j = sample(0, width - 1);
        if (gameField[i][j] == nullptr) {
            Rabbit *thisRabbit = new Rabbit(i, j);
...
int GameEngine::remainingVeggies() {
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            Veggie *test = dynamic_cast<Veggie *>(gameField[i][j]);
            if (test != nullptr) {
                num++;
            }
        }
...
void GameEngine::moveCaptain() {
    ...
    cin >> input;
    if (input.length() == 1) {
        char direction = tolower(input[0]);
        cout << "\n";
        switch (direction) {
            case 'w':
                moveCptVertical(-1);
...
void GameEngine::gameOver() {
    ...
    for (auto veggie: gameCaptain->getVeggies()) {
        cout << veggie->getVeggieName() << "\n";
    }
